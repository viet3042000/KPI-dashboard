...
/*    public static Date getTruncSysdate(EntityManager em) throws Exception {
    ...
    public static Query setParam(Query query, Map<String, Object> mapParam) {
        if (!mapParam.isEmpty()) {
            for (Map.Entry<String, Object> map : mapParam.entrySet()) {
                if (isCollection(map.getValue())) {
                    setParamInQuery(query, map.getKey(), (List) map.getValue());
                } else if(map.getValue() instanceof Date) {
                    query.setParameter(map.getKey(), (Date) map.getValue(), TemporalType.DATE);
                } else {
